//日常行为观察
function loadRcgclb(taskId,qh,url){
	$('#basecontent').load(url,function(){
		rcgcScale(taskId); //调用加载日常量表方法
		rcycyqData(qh); //调用加载异常狱情方法
	});
}
function rcgcScale(taskId){
	$.ajax({
    	url:"/oasm/ztk/rcxwgc_start_question",
    	type:"post",
    	dataType:"JSON",
    	data:{taskId:taskId},
...
//保存日常行为观察答题表
function rcgc_radioOrcbox_save(){
	var radio_option="";
	var checkbox_option="";
	var radio_score = 0;
	...
	var raletjh = $("#hidden_rcgcjh").text().trim(); //关联的警号
...
//日常观察结果表
function loadRcgcrs(taskId,url){
	$('#basecontent').load(url,function(){
		rcgcData(taskId); //调用显示异常狱情数据
	});
}
function rcgcData(taskId){
	$.ajax({
