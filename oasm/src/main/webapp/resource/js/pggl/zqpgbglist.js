...
$(function() {
	...
	// *********************************统一搜索框******************************************
    //中期报告
    $("#zqbg_ss_btn").click(function(){
   	 goPage(1, psize, "zqpgbg");
    });
});

//中期显示评估报告列表
function zqpgList(gyjq,qh,xm,jhxm,eval,stat){
	$.ajax({
		url:"/oasm/pgbgView/getViewPgbg",
		type:"post",
		dataType:"JSON",
		async:false,
		data:{pageNum : pageNum,pageSize : pageSize,type:"mdRep",
	    ...
	    success:function(data){
	    	...
	    	if(null!=data.viewPgbg&&""!=data.viewPgbg){
	    		$.each(data.viewPgbg,function(k,v){
					...
					a += "<td>" + st + "</td>";// 任务表状态
					var zqbgurl = "\"/oasm/pgbgsccxView/pgbg_zq?qh="+v.crimNo+"&&taskId="+v.taskId+"\""; //中期评估报告url
					if (v.status == 4) {
						a += "<td><a class='cs-a' onclick='loadzqbgPage(\""+v.status+"\",\""+v.taskId+"\","+zqbgurl+")'>查看报告</a></td></tr>";
					} else {
						a += "<td><a class='cs-a' onclick='loadzqbgPage(\""+v.status+"\",\""+v.taskId+"\","+zqbgurl+")'>查看进度</a></td></tr>";
					}
		    	});
	    	}else{
	    		a+="<tr>";
...
function goPage(pageNum1, psize1, clazz1) {
	...
	if(clazz="zqppgbg"){
		var gyjq = $("#zqpgbg_ss_selected").val();
		var qh = $("#zqbgqh").val();
		var xm = $("#zqbgxm").val();
		var jhxm = $("#zqbgjhxm").val();
		var eval = $("#zqbg_ss_radio input[name='wxdj']:checked").val();
		var stat = $("#zqbg_ss_statRadio input[name='pgStatus']:checked").val();
		zqpgList(gyjq,qh,xm,jhxm,eval,stat);
	}
