//列出出监结构访谈所需要的问题和因子
function loadCjftlb(taskId,url){
	$('#basecontent').load(url,function(){
		cjftScale(taskId); //加载出监访谈量表
	});
}
function cjftScale(taskId){
	$.ajax({
		url:"/oasm/ztk/cjjgft_start_question", //地址谨慎配置
		dataType:"JSON",
		type:"post",
		data:{taskId:taskId},
...
function cjjgft_da_save(){
	...
	var raletjh = $("#hidecjjgft_jh").text().trim(); //每个页面需要自己定义警号
...
//出监结构访谈答题结果信息
function loadCjftrs(taskId,url){
	$('#basecontent').load(url,function(){
		cjftrsData(taskId); //加载出监访谈答题记录
	});
}//end
function cjftrsData(taskId){
	$.ajax({
