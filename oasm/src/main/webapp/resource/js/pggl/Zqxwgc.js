function loadZqgclb(taskId,qh,url){
	$('#basecontent').load(url,function(){
		zqgcScale(taskId); //加载中期观察量表
		zqycyqData(qh); //加载中期观察异常狱情
	});
}
function zqgcScale(taskId){
	$.ajax({
    	url:"/oasm/ztk/zqxwgc_start_question",
    	type:"post",
    	dataType:"JSON",
    	data:{taskId:taskId},
...
function zqxwgc_da_save(){
	...
	var raletjh = $("#hidden_zqgcjh").text().trim(); //关联的警号
...
//中期行为观察结果信息
function loadZqgcrs(taskId,url){
	$('#basecontent').load(url,function(){
		zqgcrsData(taskId); //加载中期行为观察之后的数据
	});
}
function zqgcrsData(taskId){
	$.ajax({
