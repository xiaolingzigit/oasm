...
//入监结构访谈
function loadRjjgft(taskId,url){
	$('#basecontent').load(url,function(){ //重构方法，可不设置延时加载
		 rjjgftScale(taskId);
	}); 
}
function rjjgftScale(taskId){
	$.ajax({
		url:"/oasm/ztk/rjjgft_start_question",
		dataType:"JSON",
		type:"post",
		data:{taskId:taskId},
...
//入监结构访谈答题结果
function loadRjjgftRs(taskId,url){ //得到答题情况需要的任务编号和url
	$('#basecontent').load(url,function(){
		rjjgftRsData(taskId); //先 加载页面后调用方法
	});
}
function rjjgftRsData(taskId){
	$.ajax({
