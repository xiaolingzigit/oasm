<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>

<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>入监评估处置跟踪</title>

</head>
<body>
    <div class="return_btn" >
       <a href="#" onclick="loadnext3('/oasm/czgzView/rj_bgcz')"> <<<  返回 </a>	
    </div>
     <div class="rs_table">
        <div class="rs_table_title"><h1>入监评估处置跟踪情况</h1></div>
		  <div class="msg_div2">
		   <ul  class="xx_ul">
		   <li class="xx_li01">姓<span>名</span>：</li>
		   <li class="xx_li02">韩方奕</li>
		   <li class="xx_li01">囚<span>号</span>：</li>
		   <li  class="xx_number xx_li02" onclick="loadnext3('/oasm/dafxView/criminalinfo')">1234567890</li>
		   <li class="xx_li01">录入时间：</li>
		   <li class="xx_li02">2017/11/6</li>
		   <li class="xx_li01">监<span>区</span>：</li>
		   <li class="xx_li02">三监区-分监区1</li>
		   <li class="xx_li01">专管警察：</li>
		   <li class="xx_li02">王子需</li>
           <li  class="xx_li01">报告编号：</li>
		   <li class="xx_li02 xx_number" onclick="loadnext3('/oasm/pgbgsccxView/pgbg_rj')">6546545454101</li>
           <li class="xx_li01">得<span>分</span>：</li>
		   <li class="xx_li02" >55</li>	
		   <li class="xx_li01">危险等级：</li>
		   <li class="xx_li02" >低度危险</li>	
		   <li class="xx_li01">危险类型：</li>
		   <li class="xx_li02" >自杀</li>	
		   </ul>	
        </div>
		<div class="czgz_right">
			<div class="header_table">
				<h3>处置对策</h3>
			</div>
			<div class="lb_content_table2" >
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">罪犯危险性评估中心</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">教育改造部</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">心理矫治办公室主任</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">矫正与刑务办公室</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">指挥中心</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">狱政管理办公室</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>
				<table>
					<tr class="czdc_tr01">
						<th colspan="2">劳动改造与安全生产办公室</th>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">分类关押</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">管控措施</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">教育改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">劳动改造方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">刑罚执行方案</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">是否适用假释、暂予监外执行</td>
						<td class="czdc_td02"></td>
					</tr>
					<tr class="czdc_tr02">
						<td class="czdc_td01">安置帮教建议</td>
						<td class="czdc_td02"></td>
					</tr>
				</table>

			</div>
		</div>
		<div class="czgz_left">
			<div class="header_table">
				<h3>处置跟踪情况</h3>
			</div>
			<div class="czgzxx">
				处置情况良好
			</div>
		</div>
		<div style="text-align: center; padding-bottom: 10px">
			<button style="font-size: 20px;">下载</button>
			<button style="font-size: 20px;">打印</button>
		</div>
	</div>
	
</body>
</html>